...
function Preview({uri}:{uri:string}){
 ...
 return <VideoView player={player} style={{width:'100%',aspectRatio:9/16,maxHeight:460}} nativeControls contentFit="contain"/>;
}
...
export default function App(){
 ...
 async function start(){
  ...
  if(mode==='music'&&(!/^\d+$/.test(bpm)||+bpm<60||+bpm>200)){Alert.alert('Темп должен быть от 60 до 200 BPM');return;}
  ...
  try{
   const output=await renderer.render({clips:(mode==='speech'?clips.slice(0,1):clips).map(c=>c.uri),music:mode==='music'?music?.uri:null,mode,template,duration,bpm:+bpm||120,quality:hd?'1080':'720',language,captions,text});
 ...
 return <SafeAreaView style={s.root}><KeyboardAvoidingView style={{flex:1}} behavior={Platform.OS==='ios'?'padding':undefined}><ScrollView contentContainerStyle={s.page} keyboardShouldPersistTaps="handled">
  ...
  {captions==='manual'&&<><TextInput editable={!busy} value={text} onChangeText={setText} multiline maxLength={1200} style={[s.input,{minHeight:90}]} placeholder="Каждая строка — отдельный титр" placeholderTextColor="#697080"/><Text style={s.small}>Строки распределяются равномерно по ролику.</Text></>}
  ...
  {mode==='music'&&<View style={s.row}><Text style={[s.muted,{flex:1}]}>Темп музыки, BPM</Text><TextInput editable={!busy} value={bpm} onChangeText={setBpm} keyboardType="number-pad" maxLength={3} style={[s.input,{width:86,textAlign:'center'}]}/></View>}
  ...
  <Text style={[s.small,{marginVertical:18}]}>Монтаж работает без ПК. Во время экспорта оставьте приложение открытым. Видео обрезается по центру до 9:16.{mode==='music'?' Звук клипов заменяется музыкой; короткие клипы повторяются.':''}</Text>
  ...
  {result&&!busy&&<View style={s.panel}><Text style={s.title}>Твой ролик · {result.duration.toFixed(1)} сек</Text><Preview uri={result.uri}/><Button title="Сохранить в галерею / поделиться" onPress={()=>Sharing.shareAsync(result.uri,{mimeType:'video/mp4',UTI:'public.mpeg-4'}).catch(e=>Alert.alert('Экспорт',String(e)))} primary/><Button title="Удалить из приложения" onPress={removeResult}/></View>}
  <Text style={s.footer}>REELFORGE 0.2 · ON DEVICE</Text>
