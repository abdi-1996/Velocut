...
# VeloCut v0.5.6: UI/navigation cleanup only. Preserve editing/composition logic.

# 1) Remove floating Loop/Play/Undo/Redo capsule from inside Preview.
# Keep the top-right ellipsis menu and all preview rendering intact.
...
# 2) Replace the old playback row with a clean compact bar under Preview.
# Track-height scale is intentionally small and lives here; add-track stays available.
...
playback_repl=r'''    private var playback:some View{
        HStack(spacing:7){
            ...
            .buttonStyle(.plain)
            .accessibilityLabel("Добавить дорожку")
...
# 3) Remove the complete timeline header strip (Timeline / track slider / Speed FX / zoom buttons).
# The timeline body becomes a real vertical ScrollView so V4/V5/V6/... are reachable.
...
# 4) Let the vertical ScrollView own vertical drags. Horizontal drags still scrub time.
old_gesture=re.compile(r'''\.clipped\(\)\.contentShape\(Rectangle\(\)\)\.gesture\(DragGesture\(minimumDistance:12\)\.onChanged\{v in if timelineDragStart==nil\{timelineDragStart=model\.projectTime;model\.beginScrub\(\)\};model\.scrub\(to:\(timelineDragStart \?\? model\.projectTime\)-Double\(v\.translation\.width\)/pps\)\}\.onEnded\{v in let base=timelineDragStart \?\? model\.projectTime;let extra=Double\(v\.predictedEndTranslation\.width-v\.translation\.width\)/pps;timelineDragStart=nil;model\.endScrub\(\);withAnimation\(\.easeOut\(duration:0\.32\)\)\{model\.scrub\(to:base-Double\(v\.translation\.width\)/pps-extra\*0\.72\);model\.endScrub\(\)\}\}\)''')
new_gesture='''.clipped().contentShape(Rectangle()).simultaneousGesture(
            DragGesture(minimumDistance:10)
                ...
                .onEnded{v in
                    guard let base=timelineDragStart,abs(v.translation.width) > abs(v.translation.height) else{timelineDragStart=nil;return}
                    let extra=Double(v.predictedEndTranslation.width-v.translation.width)/pps
                    timelineDragStart=nil
                    model.endScrub()
                    withAnimation(.easeOut(duration:0.32)){
                        model.scrub(to:base-Double(v.translation.width)/pps-extra*0.72)
                        model.endScrub()
                    }
                }
        )'''
s,n=old_gesture.subn(new_gesture,s,count=1)
if n!=1:
    raise RuntimeError('timeline drag gesture missing')

# 5) Audio selection swaps the SAME bottom toolbar instead of showing a tall editor panel.
# Volume remains available as a compact menu; no separate slider section opens.
...
p.write_text(s)
print('Applied v0.5.6 vertical timeline scroll, under-preview controls and compact contextual toolbar')
