...
playback_repl=r'''    private var playback:some View{
        HStack(spacing:7){
            ...
            Slider(value:$model.trackHeightScale,in:0.65...1.8).frame(width:66).controlSize(.mini)
...
# Match the complete existing one-line canvas drag gesture regardless of inertia formatting.
gesture_line=re.compile(r'(?m)^(\s*)\}\.clipped\(\)\.contentShape\(Rectangle\(\)\)\.gesture\(DragGesture\(minimumDistance:12\).*\)\s*$')
new_gesture=r'''        }
        .clipped()
...
s,n=gesture_line.subn(new_gesture,s,count=1)
if n!=1:
    raise RuntimeError('timeline drag gesture line missing')
